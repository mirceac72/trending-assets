...
public class ChangeTransform extends PTransform<PCollection<KV<String, BigDecimal>>, PCollection<KV<String, BigDecimal>>> {
  ...
  public PCollection<KV<String, BigDecimal>> expand(PCollection<KV<String, BigDecimal>> input) {
    return input
        .apply("add-timestamp-to-values", ParDo.of(new DoFn<KV<String, BigDecimal>, KV<String,
            TimestampedValue<BigDecimal>>>() {
          @ProcessElement
          public void processElement(ProcessContext c) {
            var value = c.element().getValue();
            var timestamp = c.timestamp();
            c.outputWithTimestamp(KV.of(c.element().getKey(), TimestampedValue.of(value, timestamp)), timestamp);
          }
        }))
        .apply("group-consecutive-values", Window.<KV<String, TimestampedValue<BigDecimal>>>into(
        ...
        .apply("compute-count-and-variation",
            MapElements.into(TypeDescriptors.kvs(TypeDescriptors.strings(),
                ...
                .via(keyAndValues -> {
                  ...
                  count++;
                  TimestampedValue<BigDecimal> first = iterator.next();
                  if (!iterator.hasNext()) {
                    return KV.of(asset, CountAndChange.of(count, BigDecimal.ZERO));
                  }
                  count++;
                  TimestampedValue<BigDecimal> last = iterator.next();
                  var change = last.getTimestamp().compareTo(first.getTimestamp()) > 0
                      ? last.getValue().subtract(first.getValue())
                      : first.getValue().subtract(last.getValue());
                  if (iterator.hasNext()) {
                    count++;
                  }
